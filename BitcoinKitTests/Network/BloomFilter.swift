...
class BloomFilterTests: XCTestCase {
    func testBloomFilter() {
        ...
        do {
            var filter = BloomFilter(elementCount: 3, falsePositiveRate: 0.01, randomNonce: 0)
            ...
            let message = FilterLoadMessage(filter: Data(filter.filters), hashFuncs: filter.hashFuncs, tweak: filter.tweak, flags: 1)
            XCTAssertEqual(message.serialized().hex, Data(hex: "03614e9b050000000000000001")!.hex)
        }
        do {
            var filter = BloomFilter(elementCount: 3, falsePositiveRate: 0.01, randomNonce: 2147483649)
            filter.insert(Data(hex: "99108ad8ed9bb6274d3980bab5a85c048f0950c8")!)
            filter.insert(Data(hex: "b5a2c786d9ef4658287ced5914b37a1b4aa32eee")!)
            filter.insert(Data(hex: "b9300670b4c5366e95b2699e8b18bc75e5f729c5")!)
        ...
        do {
            var filter = BloomFilter(elementCount: 4, falsePositiveRate: 0.001, randomNonce: 100)
        ...
        do {
            ...
            let publicKey1 = PublicKey(data: Data(hex: "03cdb817b334c8e3bdc6ce3a1eae9e624cc64426eb00ef9207d2021ce6d9253a2a")!, network: .testnet)
            filter.insert(publicKey1.data)
            filter.insert(Crypto.sha256ripemd160(publicKey1.data))
            
            let publicKey2 = PublicKey(data: Data(hex: "02784addc6ceed8bbbee10829194ce17c99a6a7029b3a9e078b6f849aa91c937b5")!, network: .testnet)
